...
def read_vipers(data_dir='../data/VIPERS/'):
    ...
    survey_name = 'VIPERS'
    survey_columns = {'id': None, 'ra': 'alpha', 'dec': 'delta', 'mag': 'r', 'flag': 'zflg', 'redshift': 'zspec',
                      'redshift_error': None, 'magnitude': None}
    ...
    for url in urls:
        ...
        data = fits.open('%s/%s' % (data_dir, file_name))[1].data
        flag_z = np.bitwise_and(data[survey_columns['flag']] >= 2, data[survey_columns['flag']] <= 4)
        flag_z += np.bitwise_and(data[survey_columns['flag']] >= 12, data[survey_columns['flag']] <= 14)
        n_good = flag_z.sum()
        print 'n_z good / total = %i / %i' % (n_good, len(data))
        n_good_total += n_good
...
def read_2dfgrs(data_dir='../data/2dFGRS/'):
    ...
    download_file(survey_data['catalog_readme'], data_dir)
    ...
    data = np.loadtxt('%s/%s' % (data_dir, file_name), dtype=dt, usecols=(0, 10, 11, 12, 13, 14, 15, 16, 23, 26))[:100]
...
def read_sdss_dr10(data_dir='../data/SDSS/'):
    survey_name = '2dFGRS'
    survey_columns = {'id': 'SPECOBJID', 'ra': 'PLUG_RA', 'dec': 'PLUG_DEC', 'flag': None, 'redshift': 'Z',
                      'redshift_error': 'Z_ERR', 'magnitude': None}
    survey_data = s_data[s_data['name'] == survey_name][0].data
    ...
    data = fits.open('%s/%s' % (data_dir, file_name))
